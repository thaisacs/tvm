...
std::string DNA::DNAGenerator(std::vector<std::string> program_tokens, std::shared_ptr<Dict> dict) {
...
std::string DNA::ConvertMod2DNA(std::string mod_str, std::shared_ptr<Dict> dict, std::string task_name) {
    ...
    return this->DNAGenerator(program_tokens, dict);
}

DNA::DNA(std::string mod_str, std::shared_ptr<Dict> dict, std::string task_name) {
    this->gene = this->ConvertMod2DNA(mod_str, dict, task_name);
}
